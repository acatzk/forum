...
export default function Reactions ({ post_id, likes, likes_aggregate, handleLike, handleUnlike }) {
  ...
  return (
    <div className="inline-flex items-center py-2">
      <button onClick={liked ? () => unlike(liked.id) : like} className={ `${liked ? 'text-indigo-600' : 'text-gray-600'} appearance-none p-1  rounded-full` }>
        {liked ? (
      ...
      <span className={ `ml-1 text-sm ${liked ? 'text-indigo-600' : 'text-gray-600'}`  }>
